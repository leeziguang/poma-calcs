...
**REST API vs `confluence read`**

- The Confluence **Content REST API** returns **metadata only** unless you ask for the body. **Always append a query string** to the request URL, at minimum **`?expand=body.storage`**, so the response includes `body.storage` (otherwise `body` / `body.storage` is missing and parsers throw, e.g. `Cannot read property 'storage' of undefined`).
  - Full example (add more `expand` values comma-separated as needed):  
    `GET /rest/api/content/{id}?expand=body.storage,body.view,body.atlas_doc_format,version,space`
  - Minimum for body text: **`?expand=body.storage`** at the end of the URL path.  
  - `body.storage` is the usual canonical source; some pages expose **`body.atlas_doc_format`** instead of or alongside storage.
- **`confluence read`** does **not** accept `?expand=...` as part of the command (it is not a URL). Whether it works depends on the CLI **internally** requesting `expand` when it calls the API. If you see errors like **`Cannot read property 'storage' of undefined`**, the response likely had no `body` — use **direct REST** with `?expand=body.storage` below or upgrade `confluence-cli`.

**Fallback: curl (or any HTTP client)**

```sh
curl -sS -H "Authorization: Bearer YOUR_PAT" \
  "https://<confluence-host>/rest/api/content/123456789?expand=body.storage,body.view,body.atlas_doc_format,version,space"
```
